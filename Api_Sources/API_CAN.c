...
extern  INT16U  stand_id;//收到报文ID
extern  INT8U   m_dlc;//收到报文长度 
...
void  API_CAN_Init(void)
{
    ...
    */ 
    ...
    CANBTR1_SAMP    = SAMP_1;
    CANBTR0_BRP     = baud_rate;
    //CANBTR0_BRP     = 0;//500k             //降低波特率2016.9.13
    //CANBTR0_BRP     = 1;//250k
    //CANBTR0_BRP     = 4;//100k
...
* onther         : 新增 
--------------------------------------------------------------------------*/
void Api_LoadMsg(INT8U * data,INT8U *bufaddr,INT8U len)
{
    INT8U  i;
    for(i = 0 ; i < len ; i++)
    {
        data[i] = *((INT8U *)(bufaddr + i ));
    }
}
/*-------------------------------------------------------------------------
* Function Name  : Api_CAN_Rev
* Description    : 
* Input          : 
* Output         : None
* Return         : None
* onther         : 
--------------------------------------------------------------------------*/
void Api_CAN_Rev(void)
{
    if(g_canMsg001.ProceF) 
    {
      Api_LoadMsg((INT8U*)&GCan001msg.data[0],&g_canMsg001.Data[0],8);
      g_canMsg001.ProceF = empoty;    
    }
    
    if(g_canMsg002.ProceF) 
    {
      Api_LoadMsg((INT8U*)&GCan002msg.data[0],&g_canMsg002.Data[0],8);
      g_canMsg002.ProceF = empoty;    
    }
}
/*-------------------------------------------------------------------------
...
* onther         : 
--------------------------------------------------------------------------*/
interrupt void API_CAN_Rev_isr(void)//20210609---新增
{
    CANRIER_RXFIE = 0;
    ...
    {
      case 0x001:
          if(g_canMsg001.ProceF == empoty) 
          {
             if(m_dlc == 8)
               {
                 g_canMsg001.ProceF = msg_full;

                 Api_LoadMsg((INT8U*)&g_canMsg001.Data[0],&CANRXDSR0,m_dlc);
               }
          }
          break; 
       case 0x002:
          if(g_canMsg002.ProceF == empoty) 
          {
             if(m_dlc == 8)
               {
                 g_canMsg002.ProceF = msg_full;

                 Api_LoadMsg((INT8U*)&g_canMsg002.Data[0],&CANRXDSR0,m_dlc);
               }
          }
          break; 
        default:
