...
/**********/
void API_Set7000msTimer(INT16U Second,ClockType *T)//20210330--调试
{    
    T->Over7000msTime = Second;
    Clock.T7000msTime = null;
    T->T7000msEnable = Enable;
}

INT8U API_7000msTimerOver(ClockType *T)
{
    if((T->T7000msTime >= T->Over7000msTime)&&(T->T7000msEnable == Enable))
    {        
        T->T7000msEnable = Disable;
        T->T7000msTime = null;
        return 1;
    }
    else
...
void API_RTC_isr(void)
{        
    /*if(pSBLInput == PushDown)
    {
        if(Key.PushTime1 < 2000)
        {
            Key.PushTime1 ++;  
        }          
    }
    ...
    if(Clock.T7000msEnable)
    {
        if(Clock.T7000msTime < Clock.Over7000msTime)
        {
            Clock.T7000msTime++;
        }
    }
